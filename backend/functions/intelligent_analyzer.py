...
def perform_intelligent_comparison(famille: str, period1_data: Dict, period2_data: Dict, 
    ...
    if abs(total_pct) > 10:
        ...
        analysis.append(f"Consommation totale a {direction} de {abs(total_pct):.1f}% ({abs(total_diff):.2f} unités)")
    
    if abs(avg_pct) > 15:
        direction = "augmenté" if avg_diff > 0 else "diminué"
        analysis.append(f"Consommation moyenne a {direction} de {abs(avg_pct):.1f}%")
    ...
    if total_pct > 20:
        insights.append("⚠️ Augmentation significative de la consommation - vérifier les causes")
    elif total_pct < -20:
        insights.append("✅ Réduction importante de la consommation - efficacité améliorée")
    elif abs(total_pct) < 5:
        insights.append("📊 Consommation stable entre les deux périodes")
    ...
    if period1_variability > period2_variability * 1.5:
        insights.append("📈 Consommation plus variable dans la période récente")
    elif period2_variability > period1_variability * 1.5:
        insights.append("📉 Consommation plus stable dans la période récente")
...
def generate_intelligent_summary(famille: str, start_date: datetime, end_date: datetime, 
    ...
    summary = {
        ...
        'key_metrics': {
            'total_consumption': round(aggregates['sum'], 2),
            'daily_average': round(aggregates['mean'], 2),
            'peak_consumption': round(aggregates['max'], 2),
            'lowest_consumption': round(aggregates['min'], 2),
    ...
    if aggregates['sum'] > 0:
        ...
        if daily_avg > 200:
            insights.append("🔴 Consommation élevée détectée")
        elif daily_avg > 100:
            insights.append("🟡 Consommation modérée")
        else:
            insights.append("🟢 Consommation faible")
    ...
    if aggregates['count'] > 1:
        ...
        if variability > 2:
            insights.append("📊 Forte variabilité dans la consommation")
        elif variability < 0.5:
            insights.append("📊 Consommation très stable")
        else:
    ...
    if aggregates['max'] > aggregates['mean'] * 2:
        recommendations.append("💡 Investiguer les pics de consommation pour optimiser")
...
def analyze_weekend_pattern(daily_breakdown: Dict, start_date: datetime) -> Optional[str]:
    """Analyze weekend vs weekday consumption patterns"""
    ...
    if weekday_totals and weekend_totals:
        ...
        if diff_pct > 20:
            return "📅 Consommation plus élevée le week-end (+{:.1f}%)".format(diff_pct)
        elif diff_pct < -20:
            return "📅 Consommation plus faible le week-end ({:.1f}%)".format(diff_pct)
